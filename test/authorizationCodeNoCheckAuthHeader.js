...
describe('Authorization Code Grant Type no Authorization Header Check',function() {
    ...
    it('POST /token with grant_type="authorization_code" expect token not checking Authorization Header', function(done) {
        app.get('oauth2').options.checkAuthorizationHeader = false;
        //console.log(app.get('oauth2'));
        request(app)
            .post('/token')
            ...
            .end(function(err, res) {
                app.get('oauth2').options.checkAuthorizationHeader = true; // set back to true
                if (err) return done(err);
