...
module.exports = function(req, res) {
    ...
    async.waterfall([
        ...
        function(cb) {
            if(!req.oauth2.options.checkAuthorizationHeader){
              //must fetch client information by code
              var code = req.body.code;
              req.oauth2.model.code.fetchByCode(code, function(err, codeGet){
                if(err)
                    cb(new error.serverError('Failed to call codes::fetchByCode method'));
                else if(!code)
                    cb(new error.serverError('Code is not valid'));
                else{
                  clientId = codeGet.clientId;
                  cb();
                }
              });
            }else{
